...
CASES = ["portrait_walking_pass", "fast_motion_warping_fail"]


@pytest.mark.parametrize("case", CASES)
def test_video_eval_schema_valid(case: str) -> None:
...
@pytest.mark.parametrize("case", CASES)
def test_video_eval_scores_within_tolerance(case: str) -> None:
    inp = json.loads((INPUTS_DIR / f"{case}.json").read_text())
...
@pytest.mark.parametrize("case", CASES)
def test_video_eval_pass_threshold(case: str) -> None:
    inp = json.loads((INPUTS_DIR / f"{case}.json").read_text())
    expected = json.loads((EXPECTED_DIR / f"{case}.json").read_text())
    raw = json.dumps(inp["raw_llm_response"])
    out = parse_eval_output(raw)
