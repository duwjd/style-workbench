...
- [ ] Docker Desktop 또는 Colima + 충분한 디스크 (Postgres + MinIO 컨테이너용 ~2GB).
- [ ] Node 22+, Python 3.12+, uv 0.4+, pnpm 9+ 설치. 미설치면 Claude Code에 설치 절차 도움 요청.
...
- 루트에 docker-compose.yml (postgres 16, minio 최신, backend, frontend) 초안
...
- minio:latest (S3 호환, 9000/9001 포트, 콘솔 포함)
...
**1주차 끝 → 체크포인트**:
- [ ] `docker compose up` 으로 4개 서비스 정상 기동
...
SYSTEM_ARCHITECTURE.md §3.1 의 ReplicateAdapter 시그니처를 따라:
  ...
  - 완료 후 output URL을 다운로드 → S3(MinIO) 업로드 → S3 URL 반환
...
- 출력은 S3 (boto3 + minio) 업로드 후 URL 반환
...
- `/review` — PR 단위 리뷰
- `/security-review` — 외부 API 호출, S3 업로드, 인증 부분에 한 번씩
...
모든 단계가 끝나면 README.md 를 쓰는데, 다음을 포함해:
  ...
  - 개발 시작 커맨드 (docker compose up + pnpm dev)
  - 폴더 구조 트리 (간략)
...
| Replicate 영상 생성 응답 URL이 짧은 시간 후 만료 | succeeded 즉시 S3로 다운로드/재업로드 (어댑터 책임). DB에는 S3 URL만 저장 |
| asyncpg + SQLAlchemy 트랜잭션 hang | 같은 세션에서 `await` 누락된 곳이 있는지 점검. mypy + ruff 가 잡아줌 |
| MinIO 에 presigned URL 이 외부에서 접근 안 됨 | docker network 안에서는 `minio:9000`, 브라우저에서는 `localhost:9000` — endpoint를 두 개 분리 |
